...
import org.example.snake.dao.UserDAO;
import org.example.snake.model.User;
...
public class GameController {
    ...
    private static final int WIDTH = 24;  // 600px / 25 = 24 cases
    private static final int HEIGHT = 24; // 600px / 25 = 24 cases
    ...
    private void gameOver(String raison) {
        running = false;
        timer.stop();
        dao.updateScore(joueur.getId(), score);

        if (adminController != null) adminController.refreshTable();

        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setTitle("Game Over");
        alert.setHeaderText(raison);
        alert.setContentText("Score final : " + score + " points.");
        alert.showAndWait();
